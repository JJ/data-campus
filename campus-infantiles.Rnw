...
ni\~nas y ni\~nos expresar\'an su opini\~nn sobre si quieren hacerla o no y,
...
interesante el estudio presentado en este artículo, que examina
opciones reales, aunque reducidas a un área y rango de a\~nos
determinado. 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{img/2010.png}
\includegraphics[width=0.95\textwidth]{img/2011.png}
\caption{Chicos y chicas por edad, a\~nos 2013 a 2015}
\end{figure}
...
El panorama cambia claramente este a\~no, con m\'as chicos que chicas en
cualquier edad.
...
\caption{Chicos y chicas por edad, a\~no 2016}
\end{figure}

Finalmente, este mismo a\~no, salvo para 7 a\~nos en el que hay 5 chicas y 2
chicos, en el resto de las edades la proporci\'on de chicos es siempre
mayor, hasta ser abrumadora para 11 o m\'as a\~nos.

A continuaci\'on examinaremos, por grupos de edades, cu\'al ha sido la
evoluci\'on de esta proporci\'on.

\section{Evoluci\'on de la proporci\'on de
chicas}\label{evoluciuxf3n-de-la-proporciuxf3n-de-chicas}

<<evolucion, include=FALSE, cache=FALSE>>=
...
<<resumen, include=FALSE, cache=FALSE>>=
library(knitr)
...
La gr\'afica muestra tambi\'en, en el grupo de mayor edad, una estabilidad
